<?php
    ...
    if($_SERVER['REQUEST_METHOD']=='GET'){
        if(isset($_GET['id_credi_loan_payment'])){
            $query = "SELECT * FROM credi_loan_payment WHERE id_credi_loan_payment=".$_GET['id_credi_loan_payment'];
            $resultado = metodoGet($query);
            echo json_decode($resultado->fetch(PDO::FETCH_ASSOC));

        }else{
            $query="select * from credi_loan_payment";
            $resultado = metodoGet($query);
    ...
    if($_POST['METHOD']=='POST'){
        ...
        $payment_mount = $_POST['payment_mount'];
        $id_credi_loan = $_POST['id_credi_loan'];
